...
const ebookModels = {
  getByUserId: (userId) => {
    return new Promise((resolve, reject) => {
      db.query(
        `SELECT * FROM ebook WHERE user_id = '${userId}'`,
        (err, result) => {
          if (err) {
            return reject(err.message);
          } else {
  ...
  getById: (id) => {
    return new Promise((resolve, reject) => {
      db.query(`SELECT * FROM ebook WHERE id = '${id}'`, (err, result) => {
        if (err) {
          return reject(err.message);
        } else {
          return resolve(result.rows[0]);
  ...
  add: ({ userId, ebook }) => {
    ...
    return new Promise((resolve, reject) => {
      db.query(
        `INSERT INTO ebook (id, user_id, ebook_name) VALUES ('${id}', '${userId}', '${ebook}')  RETURNING *`,
        (err, result) => {
          if (err) {
            return reject(err.message);
          } else {
  ...
  delete: (id) => {
    return new Promise((resolve, reject) => {
      db.query(
        `DELETE FROM ebook WHERE id = '${id}' RETURNING *`,
        (err, result) => {
          if (err) {
            return reject(err.message);
          } else {
            if (result.rows.length < 1) {
              return reject("Ebook not found");
            }
            return resolve(result.rows[0]);
          }
        }
